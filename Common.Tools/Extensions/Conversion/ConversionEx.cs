...
namespace Common.Tools.Extensions.Conversion
{
    public static class ConversionEx
    {
        ...
        {
            Dictionary<string, PropertyInfo> withProps = target.GetType()
                .GetProperties()
                .GroupBy(x => x.Name)
                .Select(g => g.First())
        ...
        public static IEnumerable<T> From<T>(this DataView dataView, int pageNum, int pageCount)
        {
            ...
            void SyncPageNum()
            {
                ...
                {
                    int availablePageCount = (int)Math.Ceiling((double)recordCount / (double)pageCount);
