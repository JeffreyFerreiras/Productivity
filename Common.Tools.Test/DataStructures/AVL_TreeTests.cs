using Common.Tools.DataStructures;
...
namespace Common.Tools.Test.DataStructures
{
    [TestFixture]
    public class AvlTreeTests
    {
        private static readonly int[] SSample = { 212, 580, 6, 7, 28, 84, 112, 434 };

        private AvlTree<int> BinTreeFactory()
        {
            var bst = new AvlTree<int>();

            foreach (int x in SSample)
            {
                bst.Add(x);
            }
        ...
        public void Contains_SampleData_ReturnsTrue()
        {
            ...
            Assert.IsTrue(tree.Contains(SSample.Min()));
        }
        ...
        public void MaxTest()
        {
            ...
            Assert.IsTrue(bst.Max() == SSample.Max());
        }
