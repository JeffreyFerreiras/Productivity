...
namespace Productivity.Text
{
    public class Editor
    {
        ...
        {
            return Mask(text, count, 10);
        }

        public static string Mask(string text, int count, int totalLength)
        {
            if (text == null || count >= text.Trim().Length || count <= 0) return text;

            string mask = new string('*', totalLength - count);
